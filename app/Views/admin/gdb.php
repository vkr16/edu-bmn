...
<head>
    ...
    <title>Data Gedung Dan Bangunan</title>
...
<body class="font-nunito">
            ...
            <div class="w-100 mb-5 pb-5 mt-4">
                <div class="container-lg px-4">
                    ...
                    <div id="breadcrumb" class="mb-3">
                        <h3>Manajemen Data Gedung Dan Bangunan</h3>
                    ...
                    <div class="card rounded-1 bg-light border-0" hidden>
                        <div class="card-header bg-blue text-white d-flex justify-content-between align-items-center rounded-1">
                            <p class=" fs-6 mb-0"><i class="fa-solid fa-toolbox"></i>&nbsp; Data Gedung Dan Bangunan</p>
                            <span>
                        ...
                        <div class="card-body table-responsive">
                            <table id="tabel-gdb" class="table">
                                <thead>
                                ...
                                <tbody>
                                    <?php
                                    foreach ($gdbdata as $key => $gdb) {
                                    ?>
                                        <tr>
                                            <td><?= $key + 1; ?></td>
                                            <td><?= $gdb['kode_barang']; ?></td>
                                            <td><?= $gdb['nup']; ?></td>
                                            <td><?= $gdb['nama_barang']; ?></td>
                                            <td><?= $gdb['kuantitas']; ?></td>
                                            <td hidden><?= $gdb['perolehan']; ?></td>
                                            <td><?= $gdb['no_sk']; ?></td>
                                            <td hidden><?= $gdb['tanggal_sk']; ?></td>
                                            <td hidden><?= $gdb['instansi_sk']; ?></td>
                                            <td hidden><?= $gdb['sesuai']; ?></td>
                                            <td hidden><?= $gdb['tidak_sesuai']; ?></td>
                                            <td hidden><?= $gdb['pihak_lain']; ?></td>
                                            <td><?= $gdb['keterangan']; ?></td>
                                            <td>
                                                <button class="btn btn-sm btn-blue" style="white-space: nowrap;" onclick="gdbdetail(<?= $gdb['id']; ?>)"><i class=" fa-solid fa-circle-info"></i>&nbsp; Detail</button>
                                            </td>
                                        </tr>
                                    <?php } ?>
                                </tbody>
    ...
    <div class="modal fade" id="gdb-detail-modal" tabindex="-1" aria-labelledby="gdb-detail-modalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content rounded-1">
                <div class="modal-header">
                    <h5 class="modal-title" id="gdb-detail-modalLabel">UID : <span id="detail-uid">[ <small>Memuat Data. . . .</small> ]</span> </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>

                <div class="modal-body" id="gdb-detail-body">
                    ...
                    <div class="d-flex justify-content-start gap-3">
                        <button class="btn btn-danger rounded-1" id="gdbdata-delete-btn"><i class="fa-solid fa-trash-can"></i>&nbsp; Hapus</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        $('#sidebar-item-gdb').addClass('sidebar-active').removeClass('sidebar-item');
        ...
        function gdbdetail(id) {
            Notiflix.Block.dots('#gdb-detail-body', 'Memuat Data');
            $('#gdb-detail-modal').modal('show')

            $.post("peralatan-dan-mesin/detail", {
                    id: id
                })
                .done(function(data) {
                    Notiflix.Block.remove('#gdb-detail-body', 500);
                    console.log(data)
                    const gdbdetail = JSON.parse(data)
                    $('#detail-uid').html(gdbdetail.uid)
                    $('#detail-kodebarang').html(gdbdetail.kode_barang)
                    $('#detail-nup').html(gdbdetail.nup)
                    $('#detail-kuantitas').html(gdbdetail.kuantitas)
                    $('#detail-namabarang').html(gdbdetail.nama_barang)
                    $('#detail-nilaiperolehan').html(gdbdetail.perolehan)
                    $('#detail-nomorsk').html(gdbdetail.no_sk)
                    $('#detail-tanggalsk').html(gdbdetail.tanggal_sk)
                    $('#detail-instansisk').html(gdbdetail.instansi_sk)
                    $('#detail-sesuai').html(gdbdetail.sesuai)
                    $('#detail-tidaksesuai').html(gdbdetail.tidak_sesuai)
                    $('#detail-pihaklain').html(gdbdetail.pihak_lain)
                    $('#detail-keterangan').html(gdbdetail.keterangan)
                    $('#detail-lastedited').html(gdbdetail.last_update)
                    $('#detail-lasteditor').html(gdbdetail.last_editor)

                    // QR LINK TO EDIT
                    $('#theQR').attr('src', 'https://chart.googleapis.com/chart?cht=qr&chs=200x200&chl=<?= HOST_URL ?>/data/gdb?uid=' + gdbdetail.uid)

                    $('#qrdownload').attr('onclick', 'downloadqr(' + gdbdetail.uid + ')')
                    $('#gdbdata-delete-btn').attr('onclick', 'deletegdbdata(' + gdbdetail.id + ',' + gdbdetail.uid + ')')
                });
        }

        function deletegdbdata(id, uid) {
