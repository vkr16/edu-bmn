...
<head>
    ...
    <title>Data Konstruksi Dalam Pengerjaan</title>
...
<body class="font-nunito">
            ...
            <div class="w-100 mb-5 pb-5 mt-4">
                <div class="container-lg px-4">
                    ...
                    <div id="breadcrumb" class="mb-3">
                        <h3>Manajemen Data Konstruksi Dalam Pengerjaan</h3>
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item">Admin</li>
                                <li class="breadcrumb-item">Manajemen Data</li>
                                <li class="breadcrumb-item active" aria-current="page">Konstruksi Dalam Pengerjaan</li>
                            </ol>
                        </nav>
                    </div>
                    ...
                    <div class="card rounded-1 bg-light border-0" hidden>
                        <div class="card-header bg-blue text-white d-flex justify-content-between align-items-center rounded-1">
                            <p class=" fs-6 mb-0"><i class="fa-solid fa-toolbox"></i>&nbsp; Data Konstruksi Dalam Pengerjaan</p>
                            <span>
                        ...
                        <div class="card-body table-responsive">
                            <table id="tabel-kdp" class="table">
                                <thead>
                                ...
                                <tbody>
                                    <?php
                                    foreach ($kdpdata as $key => $kdp) {
                                    ?>
                                        <tr>
                                            <td><?= $key + 1; ?></td>
                                            <td><?= $kdp['kode_barang']; ?></td>
                                            <td><?= $kdp['nup']; ?></td>
                                            <td><?= $kdp['nama_barang']; ?></td>
                                            <td><?= $kdp['kuantitas']; ?></td>
                                            <td hidden><?= $kdp['perolehan']; ?></td>
                                            <td><?= $kdp['no_sk']; ?></td>
                                            <td hidden><?= $kdp['tanggal_sk']; ?></td>
                                            <td hidden><?= $kdp['instansi_sk']; ?></td>
                                            <td hidden><?= $kdp['sesuai']; ?></td>
                                            <td hidden><?= $kdp['tidak_sesuai']; ?></td>
                                            <td hidden><?= $kdp['pihak_lain']; ?></td>
                                            <td><?= $kdp['keterangan']; ?></td>
                                            <td>
                                                <button class="btn btn-sm btn-blue" style="white-space: nowrap;" onclick="kdpdetail(<?= $kdp['id']; ?>)"><i class=" fa-solid fa-circle-info"></i>&nbsp; Detail</button>
                                            </td>
                                        </tr>
                                    <?php } ?>
                                </tbody>
    ...
    <div class="modal fade" id="kdp-detail-modal" tabindex="-1" aria-labelledby="kdp-detail-modalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content rounded-1">
                <div class="modal-header">
                    <h5 class="modal-title" id="kdp-detail-modalLabel">UID : <span id="detail-uid">[ <small>Memuat Data. . . .</small> ]</span> </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>

                <div class="modal-body" id="kdp-detail-body">
                    ...
                    <div class="d-flex justify-content-start gap-3">
                        <button class="btn btn-danger rounded-1" id="kdpdata-delete-btn"><i class="fa-solid fa-trash-can"></i>&nbsp; Hapus</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        $('#sidebar-item-kdp').addClass('sidebar-active').removeClass('sidebar-item');
        ...
        function kdpdetail(id) {
            Notiflix.Block.dots('#kdp-detail-body', 'Memuat Data');
            $('#kdp-detail-modal').modal('show')

            $.post("peralatan-dan-mesin/detail", {
                    id: id
                })
                .done(function(data) {
                    Notiflix.Block.remove('#kdp-detail-body', 500);
                    console.log(data)
                    const kdpdetail = JSON.parse(data)
                    $('#detail-uid').html(kdpdetail.uid)
                    $('#detail-kodebarang').html(kdpdetail.kode_barang)
                    $('#detail-nup').html(kdpdetail.nup)
                    $('#detail-kuantitas').html(kdpdetail.kuantitas)
                    $('#detail-namabarang').html(kdpdetail.nama_barang)
                    $('#detail-nilaiperolehan').html(kdpdetail.perolehan)
                    $('#detail-nomorsk').html(kdpdetail.no_sk)
                    $('#detail-tanggalsk').html(kdpdetail.tanggal_sk)
                    $('#detail-instansisk').html(kdpdetail.instansi_sk)
                    $('#detail-sesuai').html(kdpdetail.sesuai)
                    $('#detail-tidaksesuai').html(kdpdetail.tidak_sesuai)
                    $('#detail-pihaklain').html(kdpdetail.pihak_lain)
                    $('#detail-keterangan').html(kdpdetail.keterangan)
                    $('#detail-lastedited').html(kdpdetail.last_update)
                    $('#detail-lasteditor').html(kdpdetail.last_editor)

                    // QR LINK TO EDIT
                    $('#theQR').attr('src', 'https://chart.googleapis.com/chart?cht=qr&chs=200x200&chl=<?= HOST_URL ?>/data/kdp?uid=' + kdpdetail.uid)

                    $('#qrdownload').attr('onclick', 'downloadqr(' + kdpdetail.uid + ')')
                    $('#kdpdata-delete-btn').attr('onclick', 'deletekdpdata(' + kdpdetail.id + ',' + kdpdetail.uid + ')')
                });
        }

        function deletekdpdata(id, uid) {
